...
contract DTERC721A is ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    /**
     * @dev _specialTransfer `tokenId` from `from` to `to`.
     *
     * Requirements:
     *
     * - `to` cannot be the zero address.
     ...
     *
     * Emits a {Transfer} event.
     * for Auction Transfer Tokens
     * Added By: FlokiBB
     */
    function _auctionTransfer(
        address from,
        address to,
        uint256 tokenId
    ) internal {
    ...
    function _setOwnersExplicit(uint256 quantity) internal {
        ...
        for (uint256 i = oldNextOwnerToSet; i <= endIndex; i++) {
            if (_ownerships[i].addr == address(0)) {
                ...
                _ownerships[i] = TokenOwnership(ownership.addr, ownership.startTimestamp, ownership.burned);
            }
        }
